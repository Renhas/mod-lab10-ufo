...
namespace Ufo
{
    static internal class Analysis
    {
        ...
        static readonly double max_distance = 1e-1;
        static readonly int max_acc = (int)1e4;
        ...
        {
            double error = AnalyseOne(accuracy);
            return error / distance < value;
        }
        ...
        static private void Draw(double[] x, double[] y) 
        {
            ...
            */
            plt.Title("Зависимость количества членов ряда от радиуса");
            plt.XLabel("Относительная погрешность");
            plt.YLabel("Количество членов ряда");
            plt.Grid(false);
            plt.SaveFig("dia.png");
        }
